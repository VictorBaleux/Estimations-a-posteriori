...
- **NOUVEAU** : Ajout d'une méthode d'intégration Monte-Carlo uniforme sur x, avec calcul d'erreur
  (MAE sur plusieurs essais) et ajout au graphe + régression linéaire (log–log).
"""
...
# ====== NOUVEAU : Monte-Carlo (uniforme sur x) ======

def monte_carlo_integral(func: Callable[[np.ndarray], np.ndarray],
                         left: float, right: float, N: int, rng: np.random.Generator) -> float:
    """Estimateur MC : (right-left) * mean(f(U)), où U ~ U([left,right])."""
    U = rng.random(N) * (right - left) + left
    return (right - left) * float(np.mean(func(U)))

def monte_carlo_convergence(func: Callable[[np.ndarray], np.ndarray],
                            left: float, right: float,
                            N0: int = 25, levels: int = 8,
                            trials: int = 21,
                            I_ref: float | None = None,
                            seed: int = 12345) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
    Retourne :
      - Ns : tailles d'échantillon
      - I_means : moyenne des estimations Monte-Carlo sur 'trials' essais indépendants
      - err_mae : MAE (mean absolute error) par N, c.-à-d. moyenne de |I_hat - I_ref| sur les 'trials'.
    Si I_ref est None, err_mae est rempli de NaN.
    """
    rng_master = np.random.default_rng(seed)
    Ns = np.array([N0 * (2**k) for k in range(levels)], dtype=int)
    I_means = np.zeros_like(Ns, dtype=float)
    err_mae = np.full_like(Ns, np.nan, dtype=float)

    for j, N in enumerate(Ns):
        # Essais indépendants (graines dérivées)
        estimates = []
        for t in range(trials):
            rng = np.random.default_rng(rng_master.integers(0, 2**63-1))
            Ihat = monte_carlo_integral(func, left, right, int(N), rng)
            estimates.append(Ihat)
        estimates = np.array(estimates, dtype=float)
        I_means[j] = float(np.mean(estimates))
        if I_ref is not None:
            err_mae[j] = float(np.mean(np.abs(estimates - I_ref)))

    return Ns, I_means, err_mae

# ====== AJOUTS : deuxième fonction et orchestrateur ======

# f1 = f sur [0,1]
def run_for_f1(show_plots: bool = True, mc_trials: int = 21):
    name = "f1"
    L, R = 0.0, 1.0
    ...
    # Monte-Carlo
    Ns_mc, I_means_mc, err_mc = monte_carlo_convergence(func_vec, L, R, N0=25, levels=8, trials=mc_trials, I_ref=I_ref)
    ...
    plt.loglog(Ns_mc,  err_mc,  marker="d", linestyle="None", label="Monte-Carlo (MAE)")
    # Régressions (log–log)
    ...
    add_regression_line(Ns_mc,  err_mc,  "MC (≈ N^-1/2)")
    plt.title(f"{name}: erreur vs nb de points (log–log)")
    plt.xlabel("Nombre de points / échantillons"); plt.ylabel("|I - I_ref| ou MAE")
    plt.grid(True, which="both", ls=":"); plt.legend()
    save_show(f"{name}_errors_loglog.png", show_plots)

    rows = []
    for n, e in zip(Ns_riem, err_riem):
        rows.append({"method": "Riemann (milieux, x)", "points": int(n), "abs_error_or_MAE": float(e)})
    for m, e in zip(Ms_lebU, err_lebU):
        rows.append({"method": "Lebesgue uniforme (y)", "points": int(m), "abs_error_or_MAE": float(e)})
    for n, e in zip(Ns_iter, err_iter):
        rows.append({"method": "Lebesgue non uniforme (itér.)", "points": int(n), "abs_error_or_MAE": float(e)})
    for n, e in zip(Ns_mc, err_mc):
        rows.append({"method": f"Monte-Carlo (MAE, {mc_trials} essais)", "points": int(n), "abs_error_or_MAE": float(e)})
...
def run_for_f2(show_plots: bool = True, mc_trials: int = 21):
    name = "f2"
    L, R = -1.0, 1.0
    ...
    Ns_riem, Is_riem, err_riem = riemann_convergence(func_vec, L, R, N0=25, levels=14, I_ref=I_ref)
    Ms_lebU, ILs_lebU, err_lebU, *_ = lebesgue_convergence(func_vec, L, R,
        Nx=300000, M0=25, levels=14, I_ref=I_ref, avoid_singular_eps=1e-6)

    # Itératif: plus d'itérations (et x-samples) pour lisser la courbe
    Ns_iter, ILs_iter, err_iter, _ = lebesgue_nonuniform_iterative_refinement(
        func_vec, L, R, Nx=300000, N0=100, n_iters=10, show_plots=show_plots, I_ref=I_ref, avoid_singular_eps=1e-6
    )

    # Monte-Carlo (attention aux singularités aux bords : l'échantillonnage uniforme reste valable
    # car l'intégrale est finie; on se contente d'éviter d'évaluer exactement en ±1 via U~(L,R)).
    Ns_mc, I_means_mc, err_mc = monte_carlo_convergence(func_vec, L, R, N0=25, levels=14, trials=mc_trials, I_ref=I_ref)
    ...
    s4, i4 = add_regression_line(Ns_mc,  err_mc,  "MC (≈ N^-1/2)")

    plt.title(f"{name}: erreur vs nb de points (log–log)")
    plt.xlabel("Nombre de points / échantillons"); plt.ylabel("|I - π| ou MAE")
    plt.grid(True, which="both", ls=":"); plt.legend()
    save_show(f"{name}_errors_loglog.png", show_plots)
...
def main():
    ...
    parser.add_argument("--mc-trials", type=int, default=21, help="nombre d'essais indépendants pour l'estimation MAE de Monte-Carlo")
    args = parser.parse_args()

    show = not args.no_show
    if args.function in ("f1", "both"):
        run_for_f1(show_plots=show, mc_trials=args.mc_trials)
    if args.function in ("f2", "both"):
        run_for_f2(show_plots=show, mc_trials=args.mc_trials)
